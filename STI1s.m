function z=STI1s(n,lambda,sigma,zeta,c,mu,beta,um,us,un,T, init,p,P,y)
%STI1s(n,lambda,sigma,f,c,mu,beta,um,us,un,T, init,p,P,y) simulates the evolution of a continuous-time behavioural SIS epidemic model with a 1-stage negotiation process
...
while time(t)<T
    ...
    if rand<=dp %contact  %SE=0, SP=1, IE=2, IP=3
        ...
        if rand<=p(i) %partner
            j=P(i);
            flag=0; %flag=0 partner, flag=1 no partner
        else %random
        ...
        state_i=x(i);
        state_j=x(j);
        if state_i==0 && state_j==1 && flag*rand<=sigma(j)
            x(j)=0;
            zsp(t)=zsp(t)-1;
            zse(t)=zse(t)+1;
        end
         if state_i==0 && state_j==2 && rand<=lambda
          ...
          if state_i==0 && state_j==3 && flag*rand<=sigma(j)
              if rand<=lambda
             x(i)=2;
             x(j)=2;
             zie(t)=zie(t)+2;
           ...
           if state_i==1 && state_j==0 && flag*rand<=sigma(j)
            x(j)=1;
            zsp(t)=zsp(t)+1;
            zse(t)=zse(t)-1;
           end
            if state_i==1 && state_j==2 && flag*rand<=sigma(j)
            x(j)=3;
            zie(t)=zie(t)-1;
            end
            ...
            if state_i==2 && state_j==3 && flag*rand<=sigma(j)
            x(j)=2;
            zie(t)=zie(t)+1;
            end
            if state_i==2 && state_j==1 && flag*rand<=sigma(j)
              if rand<=lambda
             x(j)=2;
             zie(t)=zie(t)+1;
             zsp(t)=zsp(t)-1;
          ...
          if state_i==3 && state_j==0 && flag*rand<=sigma(j)
            x(j)=1;
            zse(t)=zse(t)-1;
            zsp(t)=zsp(t)+1;
          end
            if state_i==3 && state_j==2 && flag*rand<=sigma(j)
            x(j)=3;
            zie(t)=zie(t)-1;
            end
        end
